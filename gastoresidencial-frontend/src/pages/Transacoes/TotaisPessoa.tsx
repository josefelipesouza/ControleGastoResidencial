...
export function TotaisPessoaRelatorio() {
  ...
  useEffect(() => {
    api.get<TotaisPessoa>('/Transacao/totais-por-pessoa')
      .then(res => {
        setDados(res.data);
      })
      .catch(err => {
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              <th className="p-4 font-semibold text-gray-700">Pessoa</th>
          ...
          <tbody>
            {dados.itens.map((item) => (
              <tr key={item.pessoaId} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                <td className="p-4 text-gray-800 font-medium">{item.nomePessoa}</td>
